...
class WDTokenizer:
    def __init__(self, vocab_file, label_file, data_file=None, max_freq=None, min_freq=1):
        self.pad_token = '[PAD]'
        self.unk_token = '[UNK]'
        self.start_token = '[CLS]'
        self.end_token = '[SEP]'
    ...
    def add_special_tokens(self):
        with open(self.label_file, encoding='utf-8') as f:
            for line in f:
                self.vocab.append(line.strip())
        for token in [self.triple_sep_token, self.mention_sep_token]:
            if token in self.vocab:
                self.vocab.pop(self.vocab.index(token))
    ...
    def get_triple_sep_token(self):
        return self.triple_sep_token

    def get_ele_sep_id(self):
        return self.word2idx[self.mention_sep_token]

    def get_ele_sep_token(self):
        return self.mention_sep_token
...
if __name__ == '__main__':
    tok = WDTokenizer('data/nre/vocab.txt', 'data/nre/relations.txt', 'data/nre/train.txt', min_freq=10)
    print(tok.get_vocab()[-10:])